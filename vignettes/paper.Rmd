...
make_prefix_folder <- function(prefix) {
  if (!dir.exists(file.path("results", prefix)) | regen_results) {
  if (regen_results) {
...
## Make folders for data and fig storage
make_prefix_folder("paper/data")
make_prefix_folder("paper/figs")


data_path <- file.path("results", "paper/data")
fig_path <- file.path("results", "paper/figs")
...
## Saving results
save_data <- function(df, name, path = data_path) {
...
## Reading results
read_data <- function(name, path = data_path) {
  path <- file.path(path, paste0(name, ".rds"))
...
## Saving figures
save_figure <- function(fig, name, path = fig_path) {
  
   path <- file.path(path, paste0(name, ".png")) 
   
   ggsave(path, fig, dpi = 320, width = 8, height = 8)
}
...
## See here: 
if (regen_results) {
  ets <- readRDS(here("data-raw/tb_data/tbinenglanddataclean/clean_ets_2016.rds"))
}
```
...
if (regen_results) {
  source(here("scripts/nested-missing-data.R"))
}else{
...
if (regen_results) {
  source(here("scripts/explore-variable-missingness.R"))
}else{
