...
Missing data in the ETS was highly complex with changing completeness over time and associations with multiple demographic factors. The launch of the web based ETS system was linked to reduced missing data, followed by improved completeness over subsequent years for multiple outcomes but for most outcoms this improvement then stalled. An updated data collection approach may be able to reduce missing data issues further. 

The nature of the associations could not be generalised over the outcomes considered with it being clear that individual mechanisms led to each. However, for all outcomes considered - that were well powered - there was evidence of associations with demographic variables. This indicates that missingness is likely to be not MAR and hence must be accounted for in any analysis, ideally using multiple imputatation. This means that complete case analysis should not be used on its own for this data source - or other similar surveillance data. Whilst many of the variables we considered may often be considered in analysis models - and hence usually included in imputatation models - this may not always be the case. Therefore, considerable care should be taken when specifying imputation models with other potentialy informative variables also being included.   
